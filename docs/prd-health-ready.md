...
**Status:** em revisão
**Data:** 2026-08-17
**Origem:** sessão de brainstorm sobre preparação da aplicação para operação em Kubernetes
...
**Estado atual.** A aplicação não expõe nenhum sinal de ciclo de vida para um orquestrador. Além disso, a preparação do armazenamento de eventos acontece durante a inicialização do processo e é fatal: se o banco estiver indisponível naquele instante, o processo encerra antes de existir qualquer servidor HTTP.
...
| Dependência externa indisponível | Parar de enviar tráfego, **não** reiniciar | Processo morre no início → reinícios em ciclo |
...
| **Processo da aplicação** | Instância em execução | Responde aos dois sinais. Mantém, em memória, o conhecimento de se conseguiu preparar o armazenamento de eventos desde que iniciou. |
...
Dado que o armazenamento de eventos nunca foi preparado com sucesso nesta instância
Quando o orquestrador consulta `/health`
Então a resposta tem código de status `200`.
...
Dado que o armazenamento de eventos foi preparado com sucesso nesta instância e o banco tornou-se indisponível em seguida
...
**P8 — Falha de preparo persiste até reinício**
Dado que a preparação do armazenamento de eventos falhou durante a inicialização desta instância
Quando o banco volta a ficar disponível
Então `/ready` continua respondendo `503` enquanto esta instância existir, e a recuperação depende de o orquestrador substituí-la.
*(Consequência aceita: o custo é um reinício que o orquestrador executa automaticamente; o benefício é não haver mecanismo de auto-recuperação escondido dentro de uma verificação de saúde.)*
...
**P10 — Falha de preparo é registrada**
Dado que a preparação do armazenamento de eventos falha durante a inicialização
Quando o processo continua sua inicialização
Então a falha e sua causa ficam registradas em log em nível de erro, permitindo diagnóstico posterior.
...
- **Sem alteração de comportamento do produto.** Nenhuma funcionalidade de eventos muda de comportamento observável em consequência desta feature — exceto a inicialização, que deixa de ser fatal quando o banco está indisponível.
...
| Item | Por quê |
|---|---|
| **Empacotamento e manifestos de deploy** (imagem de container, configuração das sondas no orquestrador) | Decidido explicitamente: esta entrega cobre apenas o comportamento da aplicação. Sem os manifestos os endpoints não são consumidos por ninguém — isso é reconhecido como fase seguinte, não como omissão. |
...
| **Auto-recuperação do preparo do armazenamento** (nova tentativa em segundo plano ou dentro da verificação) | Decisão consciente (ver P8): o orquestrador já substitui a instância; embutir recuperação numa verificação de saúde cria efeito colateral escondido e comportamento difícil de prever com várias instâncias. |
| **Gestão de versionamento de schema (migrations)** | O preparo do armazenamento na inicialização é frágil com várias instâncias, mas substituí-lo é problema próprio, com risco e escopo próprios. |
...
3. Iniciando a aplicação com o banco parado: o processo permanece em execução (não encerra), `/health` responde `200`, `/ready` responde `503`, e o log contém um registro de erro descrevendo a falha de preparo.
...
5. Reativando o banco após o cenário 3 (preparo nunca concluído): `/ready` permanece `503`; após reiniciar a aplicação com o banco disponível, `/ready` responde `200`.
